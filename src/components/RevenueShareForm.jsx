...
function RevenueShareForm({ data, setData }) {
  ...
  const downloadTemplate = () => {
    ...
    const headers = isFlatFee 
      ? ['Nama Pemeriksaan', 'Tarif (Rp)', 'Volume per Tahun', 'Flat Fee (Rp)']
      : ['Nama Pemeriksaan', 'Tarif (Rp)', 'Volume per Tahun']
    
    const sampleData = isFlatFee
      ? [
          ['Darah Rutin', '150000', '68664', '50000'],
          ['Creatinin', '150000', '32208', '50000'],
          ['Urea / BUN', '150000', '30624', '50000']
        ]
      : [
    ...
    const filename = isFlatFee ? 'template_revenue_sharing_flatfee.csv' : 'template_revenue_sharing.csv'
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-xl border border-purple-200">
        ...
        <div className="mt-3 p-3 bg-white rounded-lg border border-purple-100">
          <p className="text-xs font-semibold text-purple-700 mb-1">Pilihan Metode:</p>
          <ul className="text-xs text-gray-600 space-y-1">
            <li>• <span className="font-medium">Metode Persentase:</span> RS mendapat porsi % dari tarif, sisanya untuk pihak ketiga</li>
            <li>• <span className="font-medium">Metode Flat Fee:</span> RS bayar nominal tetap per pemeriksaan ke pihak ketiga, sisanya menjadi pendapatan RS</li>
          </ul>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <select
              value={data.calculationMethod || 'percentage'}
              onChange={(e) => setData({ ...data, calculationMethod: e.target.value })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
            >
              <option value="percentage">Metode Persentase - RS mendapat % dari tarif</option>
              <option value="flatFee">Metode Flat Fee - RS bayar nominal tetap per pemeriksaan</option>
            </select>
            <p className="text-xs text-gray-500 mt-1">
              {data.calculationMethod === 'flatFee' 
                ? 'RS membayar biaya tetap per pemeriksaan ke pihak ketiga, sisanya menjadi pendapatan RS'
                : 'RS mendapat persentase dari tarif pemeriksaan, sisanya untuk pihak ketiga'}
            </p>
          </div>
      ...
      <div className="bg-white p-6 rounded-xl border border-gray-200">
        ...
        {showAddProcedure && (
          <div className="mb-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
            ...
            <div className={`grid grid-cols-1 gap-3 ${data.calculationMethod === 'flatFee' ? 'md:grid-cols-4' : 'md:grid-cols-3'}`}>
              <input
                type="text"
          ...
          <table className="w-full text-sm">
            <thead className="bg-purple-50 border-b-2 border-purple-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-semibold text-gray-700">
                  {data.calculationMethod === 'flatFee' ? 'Pendapatan RS (Rp)' : 'Pendapatan RS (Rp)'}
                </th>
            ...
            <tfoot className="bg-purple-100 font-semibold">
              <tr>
                ...
                {data.calculationMethod === 'flatFee' && <td></td>}
